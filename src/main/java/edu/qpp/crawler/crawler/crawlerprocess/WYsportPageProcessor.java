 ...
 */
public class WYsportPageProcessor implements PageProcessor {

    private Site  site = Site.me().setRetryTimes(3).setSleepTime(1000);

    @Override
    public void process(Page page) {
        page.addTargetRequests(page.getHtml().links().regex("http://sports\\.163\\.com/\\d+/\\d+/\\d+/\\w+.html").all());
        page.putField("url",page.getRequest().getUrl());
        page.putField("digest",page.getHtml().xpath("head/title"));
        page.putField("theme","体育");
...
/*    public static void main(String[] args){
        ...
        Spider.create(wangYiPageProcessor).addUrl("http://sports.163.com/").addPipeline(new MysqlPipeline()).runAsync();
    }*/
}
