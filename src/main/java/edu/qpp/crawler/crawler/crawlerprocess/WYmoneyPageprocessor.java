 ...
 */
public class WYmoneyPageprocessor implements PageProcessor {

    private Site site = Site.me().setRetryTimes(3).setSleepTime(1000);

    @Override
    public void process(Page page) {
        page.addTargetRequests(page.getHtml().links().regex("http://money\\.163\\.com/\\d+/\\d+/\\d+/\\w+.html").all());
        page.putField("url",page.getRequest().getUrl());
        page.putField("digest",page.getHtml().xpath("head/title"));
        page.putField("theme","财经");
...
/*    public static void main(String[] args){
        ...
        Spider.create(wangYiPageProcessor).addUrl("http://money.163.com/").addPipeline(new MysqlPipeline()).runAsync();
    }*/
}
